...
class Donor:
    ...
    def new_donor(donor_info: dict):
        ...
        columns, values_len, values = crate_query_text(donor_info)
    ...
    def try_exist(user_id: int):
        ...
        with pg.DB(**CONFIG_PARAMS) as conn:
            count = conn.query(query_text, user_id).getresult()
            return count[0][0] == 1
    ...
    def update_with_data(donor_data: dict):
        if donor_data:
            ...
            columns, values_len, values = crate_query_text(donor_data)
...
class Request:
    ...
    def update_request(request_info,request_id):
        ...
        columns, values_len, values = crate_query_text(request_info)
